package com.wayfinder.rinno.wayfinderentregafinal.fastscroller;
...
import com.wayfinder.rinno.wayfinderentregafinal.adapters.AlphabetAdapter;
...
public class RecyclerViewFastScroller extends LinearLayout {
    ...
    public void setRecyclerView(RecyclerView recyclerView, List<AlphabetItem> alphabetItems) {
        ...
        RecyclerView.OnScrollListener onScrollListener = new RecyclerView.OnScrollListener() {
            ...
            public void onScrolled(final RecyclerView recyclerView, final int dx, final int dy) {
                ...
                float proportion = (float) verticalScrollOffset / ((float) verticalScrollRange - height * 2);
    ...
    private void setRecyclerViewPosition(float y) {
        if (recyclerView != null) {
            final int itemCount = alphabetItems.size();
            float proportion;
            if (handle.getY() == 0)
                proportion = 0f;
            ...
            try {
                ...
                final String bubbleText = alphabetItems.get(targetPos).word;
                if (bubble != null)
                    letra = bubbleText;
    ...
    private void alphabetTouchEventOnItem(int position) {
        ...
        String iLe = alphabets.get(position).word;
        int index = 0;
        for (Stores sto : RGlobal.tiendas) {

            if (sto.getName().startsWith(iLe)) {




                takeRecyclerViewScrollToAlphabetPosition(index);
                try {
                    if (bubble != null)
                        bubble.setText(iLe);
                    setAlphabetWorkSelected(iLe);

                } catch (Exception ignored) {
                }
                break;
            } else if (iLe.equals("#")) {
                takeRecyclerViewScrollToAlphabetPosition(0);
                try {
                    if (bubble != null)
                        bubble.setText(sto.getName());
                    setAlphabetWorkSelected(iLe);
    ...
    private class OnAlphabetItemClickListener implements AlphabetAdapter.OnItemClickListener {
        ...
        public void OnItemClicked(int alphabetPosition, int position) {
            ...
            String iLe = alphabets.get(alphabets.get(position).position).word;
            int index = 0;
            for (Stores sto : RGlobal.tiendas) {
                ...
                if (sto.getName().startsWith(iLe)) {
                    ...
                    try {
                        if (bubble != null)
                            bubble.setText(sto.getName());
                        setAlphabetWorkSelected(sto.getName());
